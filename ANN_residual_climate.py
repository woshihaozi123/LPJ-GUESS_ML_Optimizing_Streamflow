...
if __name__ == '__main__':
    ...
    Random_search.fit(train_x_tensor, train_y_r_tensor, callbacks=[EarlyStopping(monitor='loss', patience=10, verbose=1)])  # 使用早停法
    ...
    MLR.write_to_excel_file( columns_trn, combined_data_trn, r'.\output\ann_rc_Training.xlsx', sheet_name = 'Sheet1')
    MLR.write_to_excel_file(columns_tst, combined_data_tst, r'.\output\ann_rc_Test.xlsx', sheet_name='Sheet1')
    index=MLR.calIndex(test_y, prediction_tst.ravel())
    combined_data_index = list(zip([ 'NSE','R', 'PBIAS', 'RMSE', 'RMSEper', 'MAE', 'MAEper', 'PAE'], index))
    MLR.write_to_excel_file(['index','value'],combined_data_index , r'.\output\ann_rc_Index.xlsx', sheet_name='Sheet1')
    print('index:', index)
# ----------------- plot -------------------
    # 设置图片的宽度和高度（单位为英寸）
    ...
    # 保存图像到文件
    plt.savefig(r'.\output\ann_residual_cliamte.png', dpi=600)
    plt.show()
