...
if __name__ == '__main__':
    ...
    Random_search = RandomizedSearchCV(clf_xgb, param_distributions=xgb_params, n_iter=30, cv=5,
    ...
    MLR.write_to_excel_file( columns_trn, combined_data_trn, r'.\output\XGB_rc_Training.xlsx', sheet_name = 'Sheet1')
    MLR.write_to_excel_file(columns_tst, combined_data_tst, r'.\output\XGB_rc_Test.xlsx', sheet_name='Sheet1')
    index=MLR.calIndex(test_y, prediction_tst.ravel())
    combined_data_index = list(zip([ 'NSE','R', 'PBIAS', 'RMSE', 'RMSEper', 'MAE', 'MAEper', 'PAE'], index))
    MLR.write_to_excel_file(['index','value'],combined_data_index , r'.\output\XGB_rc_Index.xlsx', sheet_name='Sheet1')
    print('index:', index)
# ----------------- plot -------------------
    # 设置图片的宽度和高度（单位为英寸）
    ...
    # 保存图像到文件
    plt.savefig(r'.\output\XGB_residual_cliamte.png', dpi=600)
    plt.show()
