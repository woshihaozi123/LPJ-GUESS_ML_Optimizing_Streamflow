...
if __name__ == '__main__':
    ...
    test_y_r_tensor = torch.tensor(test_y_r, dtype=torch.float32)
    ...
    for i in range(n_iter_search):
        ...
        with torch.no_grad():
             # 假设 val_x_tensor 和 val_y_r_tensor 是验证集数据
            val_output = lstm_model(test_x_tensor)  # 前向传播
            val_output = val_output.view(test_y_r_tensor.shape)
            val_loss = loss_function(val_output.squeeze(), test_y_r_tensor.squeeze())
    ...
    for epoch in range(max_epochs):
        ...
        output = output.view(train_y_r_tensor.shape)
        loss = loss_function(output.squeeze(), train_y_r_tensor.squeeze())
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
    ...
    with torch.no_grad():
        predictive_y_r_for_training = optimal_lstm_model(train_x_tensor)
        predictive_y_r_for_training = predictive_y_r_for_training.view(-1, OUTPUT_FEATURES_NUM).data.numpy()
        predictive_y_r_for_testing = optimal_lstm_model(test_x_tensor)
    ...
    # lstm_model = LstmRNN(INPUT_FEATURES_NUM, 30, output_size=OUTPUT_FEATURES_NUM, num_layers=3)  # 16 hidden units
    ...
    MLR.write_to_excel_file(columns_trn, combined_data_trn, '.\output\LSTM_rc_Train.xlsx', sheet_name='Sheet1')
    MLR.write_to_excel_file(columns_tst, combined_data_tst, '.\output\LSTM_rc_Test.xlsx', sheet_name='Sheet1')
